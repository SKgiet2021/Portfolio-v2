  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const chatHistory: ChatMessage[] = messages
        .filter(
          (m) =>
            m.sender !== "ai" || m.text !== "👋 Hello! I'm your AI assistant."
        )
        .map((m) => ({
      ...
      await sendChatMessage(chatHistory, (chunk) => {
        ...
        setMessages((prev) => {
          ...
          if (lastMsg.sender === "ai" && prev.length > 1) {
      ...
      setMessages((prev) => [
        ...
        {
          sender: "ai",
          text: "Sorry, I'm having trouble connecting to the backend. Please make sure the RAG server is running on port 3000.",
        },
      ]);
      setIsTyping(false);
    }
    ...
    >
      ...
      <div className="relative flex flex-col w-full h-full rounded-xl border border-white/10 overflow-hidden bg-black/90 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-2 p-3 border-t border-white/10 relative z-10">
          <input
            className="flex-1 min-w-0 px-3 py-2 text-sm bg-black/50 rounded-lg border border-white/10 text-white focus:outline-none focus:ring-1 focus:ring-white/50"
