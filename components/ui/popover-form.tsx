      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute outline-none z-[9999]"
            ref={ref}
            style={{ borderRadius: 12, width, height }}
          >
            {/* Glow container - thinner glow */}
            <motion.div
            ...
            >
              <GlowEffect
                colors={["#00f0ff", "#ff00ff", "#ffff00", "#00ff00"]}
                mode="colorShift"
                blur="strong"
                duration={4}
                scale={1.0}
            ...
            {/* Form content with solid background */}
            <div className="relative h-full w-full rounded-[10px] border bg-muted shadow-[0_0_0_1px_rgba(0,0,0,0.08),0px_1px_2px_rgba(0,0,0,0.04)] p-1 overflow-hidden">
              <motion.span
                aria-hidden
                className="absolute left-4 top-[17px] text-sm text-muted-foreground data-[success]:text-transparent z-30"
...
export function PopoverFormButton({
  ...
  return (
    <button
      type="submit"
      className="relative ml-auto flex h-6 w-26 items-center justify-center overflow-hidden rounded-md bg-gradient-to-b from-primary/90 to-primary px-3 text-xs font-semibold text-primary-foreground shadow-[0_0_1px_1px_rgba(255,255,255,0.08)_inset,0_1px_1.5px_0_rgba(0,0,0,0.32),0_0_0_0.5px_#1a94ff]"
    >
      <AnimatePresence mode="popLayout" initial={false}>
        ...
        >
          {loading ? (
            <TextMorph>Submitting...</TextMorph>
          ) : (
...
export function PopoverFormSuccess({
  ...
  return (
    <>
      ...
      <p className="text-sm text-muted-foreground max-xs text-pretty mx-auto text-center">
        {description}
      </p>
    </>
  );
